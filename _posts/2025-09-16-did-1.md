...
- How-to
    ...
    - DID를 인과추론 관점에서 다시 보면, 측정이 불가능한 시간에 따른 변화량(T’A-TB)을 counterfactual과 유사한 control group의 시간에 따른 변화량(CA-CB)으로 치환하여 궁극적으로 얻고자 하는 treatment effect를 측정할 수 있다.
    - 따라서 DID 분석의 신뢰도는 control group과 counterfactual의 유사성에 달려있다. 다만 DID에서는 counterfactual 전체가 아닌 시간에 따른 변화량만을 치환하므로 변화량에 대한 유사성만 증명하면 된다.
        
    <img src="/assets/images/causal_inference/did-1-1.png" alt="인과추론의 데이터과학" style="width:70%; height:auto; display:block; margin:0 auto;">

    <img src="/assets/images/causal_inference/did-1-2.png" alt="인과추론의 데이터과학" style="width:70%; height:auto; display:block; margin:0 auto;">        
...
1. Parallel trend assumption
    - DID의 기본 가정으로 두 그룹이 시간에 따른 변화 트렌드가 같아야 한다는 가정
    - Parallel Pre-trend는 시간의 변화량과 변화 패턴의 유사성만 충족하면 되므로 상대적으로 증명이 쉽다. 그래프를 통해 Treatment 이전/이후의 트렌드를 확인하는 방식이 일반적이며, staggered treatment 등 시각적으로 확인하기 어려운 경우 DID event study를 통해 간접적인 통계적 검정이 가능.
    <img src="/assets/images/causal_inference/did-1-3.png" alt="인과추론의 데이터과학" style="width:70%; height:auto; display:block; margin:0 auto;">    
    - Parallel Pre-trend assumption이 충족되었을지라도 Parallel post-trend가 보장되지는 않는다(Parallel Pre-trends $\subset$ Parallel Trends).
...
- Parallel pre-trend는 covariates와 fixed effect를 제어함으로써 보정할 수 있으나, 모두 제약이 있다.
...
- 또는 functional form을 변환함으로써 Pre-trend parallel trend가 성립할 수도 있다.
